...
public class ObservableMessageProducer implements DisposableBean {
	...
	@SuppressWarnings({"unchecked", "rawtypes"})
	public ObservableMessageProducer(Observable outputStream, MessageChannel messageChannel) {
		subscription = outputStream.subscribe(new Action1<Object>() {
			...
			public void call(Object outputObject) {
				if (ClassUtils.isAssignable(Message.class, outputObject.getClass())) {
					messageChannel.send((Message) outputObject);
				}
				else {
					messageChannel.send(MessageBuilder.withPayload(outputObject).build());
				}
			}
